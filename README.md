...
| 原始格式 | Markdown格式 | 说明 |
|---------|-------------|------|
| 标题1-6 | `# - ######` | 自动识别标题级别（H1-H6） |
| 粗体 | `**文本**` | 支持strong和b标签 |
| 斜体 | `*文本*` | 支持em和i标签 |
| 链接 | `[文本](URL)` | 自动提取链接地址 |
| 图片 | `![alt](URL)` | 图片转为base64格式 |
| 表格 | `\| Header \|\n\| --- \|\n\| Data \|` | 支持标准表格格式 |
| 有序列表 | `1. 2. 3.` | 保持编号顺序，自动编号 |
| 无序列表 | `- - -` | 使用破折号格式 |
| 引用 | `> 文本` | 多行引用支持，自动换行 |
| 代码 | `` `代码` `` | 行内代码格式 |
| 代码块 | ````代码```` | 多行代码块，支持语法高亮 |
...
- **表格样式**：不支持复杂的CSS样式和合并单元格
- **嵌套表格**：不支持表格内嵌套表格
- **复杂布局**：某些复杂Word布局可能无法完美转换
- **特殊格式**：部分高级格式可能需要手动调整
